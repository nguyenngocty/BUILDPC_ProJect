...
const getBankInfo = async (orderId) => {
  if (
    typeof Order.getBankInfo === "function"
  ) {
    const result =
      await Order.getBankInfo(orderId);
...
exports.createOrder = async (
  ...
  try {
    ...
    const order =
      await Order.createFromCart(data);
    ...
    if (
      ...
      if (
        Number(momoResult.resultCode) !== 0 ||
        !momoResult.payUrl
      ) {
        return res.status(400).json({
          success: false,
...
exports.getReorderCheckout = async (
  req,
  res,
  next
) => {
...
exports.createReorderCheckout = async (
  req,
  res,
  next
) => {
