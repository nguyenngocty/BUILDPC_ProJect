...
const PcPart = {
  async getAll({ keyword = "", type_id = "", product_id = "", is_visible = "", page = 1, limit = 20 } = {}) {
    const where = ["pc_parts.deleted_at IS NULL"];
    const params = [];

    if (keyword) {
      where.push("(products.name LIKE ? OR pc_part_types.type_name LIKE ? OR pc_part_types.type_code LIKE ?)");
    ...
    const data = await query(
      `SELECT
       ...
       FROM pc_parts
       INNER JOIN pc_part_types ON pc_part_types.id = pc_parts.type_id AND pc_part_types.deleted_at IS NULL
       INNER JOIN products ON products.id = pc_parts.product_id AND products.deleted_at IS NULL
       ${whereSql}
  ...
  async getById(id) {
    const rows = await query(
      `SELECT
         ...
         products.status AS product_status
       FROM pc_parts
       INNER JOIN pc_part_types ON pc_part_types.id = pc_parts.type_id
       INNER JOIN products ON products.id = pc_parts.product_id
       WHERE pc_parts.id = ? AND pc_parts.deleted_at IS NULL
       LIMIT 1`,
      [id]
    );
    return rows[0] || null;
  ...
  async create(data) {
    const result = await query(
      `INSERT INTO pc_parts (type_id, product_id, specifications, is_visible)
       VALUES (?, ?, ?, ?)`,
      [
        data.type_id,
        data.product_id,
  ...
  async update(id, data) {
    ...
    ["type_id", "product_id", "specifications", "is_visible"].forEach((field) => {
      if (Object.prototype.hasOwnProperty.call(data, field)) {
        fields.push(`${field} = ?`);

        if (["type_id", "product_id", "is_visible"].includes(field)) {
    ...
    await query(
      `UPDATE pc_parts SET ${fields.join(", ")} WHERE id = ? AND deleted_at IS NULL`,
      params
    );
  ...
  async remove(id) {
    return query(
      `UPDATE pc_parts SET deleted_at = NOW() WHERE id = ? AND deleted_at IS NULL`,
      [id]
    );
  },
  ...
  async existsType(typeId) {
    const rows = await query(
      `SELECT id FROM pc_part_types WHERE id = ? AND deleted_at IS NULL LIMIT 1`,
      [typeId]
    );
