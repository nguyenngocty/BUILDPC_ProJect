...
const Coupon = {
  async getAll({ keyword = "", type = "", status = "", page = 1, limit = 10 }) {
    const where = ["1 = 1"];
    const params = [];
  ...
  async create(data) {
    const result = await query(
      `INSERT INTO coupons
        (code, type, value, min_order, start_date, end_date, quantity, used_count, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        data.code,
        data.type,
        data.value,
        normalizeInt(data.min_order, 0),
        data.start_date || null,
        data.end_date || null,
        normalizeInt(data.quantity, 0),
        normalizeInt(data.used_count, 0),
  ...
  async update(id, data) {
    ...
    allowFields.forEach((field) => {
      if (Object.prototype.hasOwnProperty.call(data, field)) {
        ...
        if (["value", "min_order", "quantity", "used_count"].includes(field)) {
  ...
  async remove(id) {
    return query(`DELETE FROM coupons WHERE id = ?`, [id]);
  },
};
